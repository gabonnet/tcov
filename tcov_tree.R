...
gen_tree <- function(tree){
  ...
  ## Deaths from COVID within the branch 
  ...
  ## Deaths from treatment side effects in the branch 
  ...
  ## DALYs from treatment side effects in the branch 
  ...
  ## Time needing ICU from COVID for hospitalized patients, including for cases where it is not available
  ...
  ## Compute the probability for each scenario to be the most cost-effective 
  
  a <- list()
  
  # If we include scenarios in which mechanical ventilation is unavailable
  if(NoVenti==TRUE)
  ...
  a[[1]] <- a2[ , .(perIL6=.N/Nsamples) , by = .(Optscen_VentiIL6 , pcov,country,Income_level)]
  a[[2]] <- a2[ , .(perVenti=.N/Nsamples) , by = .(Optscen_Venti , pcov,country,Income_level)] 
  a[[3]] <- a2[ , .(perOxy=.N/Nsamples) , by = .(Optscen_Oxy , pcov,country,Income_level)]
  }
  
  # If we do not include scenarios in which mechanical ventilation is unavailable
